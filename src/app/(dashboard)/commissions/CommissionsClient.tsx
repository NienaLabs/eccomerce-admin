...
import {
  ...
  ReceiptText, Filter, Search, RefreshCw
} from "lucide-react";
...
export function CommissionsClient({ initialData, token }: { initialData: VendorSummary[]; token: string }) {
  ...
  const handleSetCustomRate = async (vendorId: string, currentRate: number) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/admin/commissions/rate`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ vendor_id: vendorId, commission_rate: rate }),
      });
      if (res.ok) {
        await refresh();
        setRateInputs(prev => { const n = {...prev}; delete n[vendorId]; return n; });
        alert(`Commission rate for vendor updated to ${rate}%`);
      } else {
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ink-muted">@{vendor.store_slug} · Rate: {vendor.commission_rate}%</p>
                  </div>
                  {vendor.is_verified && (
                  ...
                  <div className="p-4 bg-surface-soft/30">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
                      ...
                      <div className="flex items-center gap-2 bg-surface p-2 rounded-lg border border-surface-muted shadow-sm">
                        ...
                        <button
                          onClick={() => handleSetCustomRate(vendor.vendor_id, vendor.commission_rate)}
